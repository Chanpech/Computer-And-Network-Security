...
static int myNodeNum = 4;
/* students to write the following two routines, and maybe some others */

void rtinit4() {
    // test for bo chen
     //   printf("node 4 initial time: %f\n", clocktime);
    printf("At time t=%f rtinit4() called\n", clocktime);
    struct RoutePacket packet;
    packet.sourceid = 4;
    //UNREACHABLE
    neighbor4 = getNeighborCosts(4);
    for(int i = 0; i < neighbor4->NodesInNetwork; i++){ //Node number from 0 to MAX_NODES
        for(int j = 0; j < neighbor4->NodesInNetwork; j++){
            if(i == j){
                dt4.costs[i][j] = neighbor4->NodeCosts[i];
            }else{
                dt4.costs[i][j] = INFINITY;
            }
            // packet.mincost[i] = dt4.costs[i][j];
            
            packet.mincost[i] = neighbor4->NodeCosts[i];
        
        }
    }
    printf("Entire Node in Network is %d\n", neighbor4->NodesInNetwork);
    printdt4(0, neighbor4, &dt4);
    ...
    int i;
    int updateOccur = 0;
    int TotalNodes = neighbor4->NodesInNetwork;     // Total nodes in network
    int NumberOfNeighbors = 0;  //was 0          // How many neighbors
    int Neighbors[MAX_NODES];                      // Who are the neighbors
    // Determine our neighbors 
    for (i = 0; i < TotalNodes; i++ )  {
        if (( neighbor4->NodeCosts[i] != INFINITY ) && i != 0 )  {
