...
void rtinit1() {
    ...
    //It should initialize the distance table in node 0 to reflect the direct costs to it
    ...
    printf("\nAt time t=%f rtinit1() called\n", clocktime);
    struct RoutePacket packet;
    packet.sourceid = 1;
    neighbor1 = getNeighborCosts(1);
    for(int i = 0; i < neighbor1->NodesInNetwork; i++){ //Node number from 0 to MAX_NODES
        for(int j = 0; j < neighbor1->NodesInNetwork; j++){
            ...
            packet.mincost[i] = neighbor1->NodeCosts[i];
        }
    }
    printf("Entire Node in Network is %d\n", neighbor1->NodesInNetwork);
    printdt1(myNodeNum, neighbor1, &dt1);
    ...
    int i;
    int updateOccur = 0;
    int TotalNodes = neighbor1->NodesInNetwork;     // Total nodes in network
    int NumberOfNeighbors = 0;  //was 0          // How many neighbors
    int Neighbors[MAX_NODES];                      // Who are the neighbors
    // Determine our neighbors 
    for (i = 0; i < TotalNodes; i++ )  {
        if (( neighbor1->NodeCosts[i] != INFINITY ) && i != myNodeNum )  {
...
void rtupdate1( struct RoutePacket *rcvdpkt ) {
    ...
    //     //printf("Before DistTable: %d\n", dt1.costs[rcvdpkt->sourceid][i]);
    //     if(dt1.costs[rcvdpkt->sourceid][i] > rcvdpkt->mincost[i]  && rcvdpkt->mincost[i] != 0){
    ...
    //         mypktptr->mincost[i] = dt1.costs[rcvdpkt->sourceid][i];
    //     }
    // }
    ...
    //     printf("dt1.cost: %d \t rcvdpkt->minCost: %d ", dt1.costs[rcvdpkt->sourceid][i],rcvdpkt->mincost[i]);
        
    //     //Find the distance from neighbor rcvdpkt->sourceid
    //     int firstPath = dt1.costs[rcvdpkt->sourceid][i];
    ...
    //     printf("Summed to %d\n", summedPath);
    //     if( dt1.costs[rcvdpkt->sourceid][i] > rcvdpkt->mincost[i]  && rcvdpkt->mincost[i] != 0 && summedPath > 0){
    //         updateOccur = 1;
    //         dt1.costs[rcvdpkt->sourceid][i] = summedPath;
