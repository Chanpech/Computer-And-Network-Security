...
dest 3|  9999  9999     7
   ...
   struct NeighborCosts {
    ...
    printf("dt0.cost[source][source]: %d dt0.cost[source][dest]: %d rcvdpkt->minCost[dest]: %d\n",dt0.costs[rcvdpkt->sourceid][rcvdpkt->sourceid], dt0.costs[rcvdpkt->sourceid][rcvdpkt->destid],rcvdpkt->mincost[rcvdpkt->destid]);
    int firstPath = dt0.costs[rcvdpkt->sourceid][rcvdpkt->sourceid];
    int secondPath = rcvdpkt->mincost[rcvdpkt->destid];
    updateOccur = 1;
    int sumCosts = firstPath + secondPath;
    printf("sumcosts %d ", sumCosts);
    dt0.costs[2][rcvdpkt->sourceid] = sumCosts; 
    mypktptr->mincost[rcvdpkt->destid] = sumCosts;
    printf("dt0 after: %d \n", dt0.costs[rcvdpkt->sourceid][rcvdpkt->destid]);
    // if(firstPath != INFINITY && secondPath != INFINITY){
    ...
    for(i = 0; i < neighbor0->NodesInNetwork; i++){
        int neighbor = Neighbors[0];
         printf("Neighbor %d\n", neighbor);
         // printf("dt0.cost[i][source]: %d dt0.cost[i][dest]: %d rcvdpkt->minCost: %d \n", dt0.costs[i][rcvdpkt->sourceid],dt0.costs[i][rcvdpkt->destid], rcvdpkt->mincost[i]);
    ...
    //     printf("dt0.cost[source]: %d dt0.cost[dest]: %d \t rcvdpkt->minCost: %d ", dt0.costs[rcvdpkt->sourceid][i],dt0.costs[rcvdpkt->destid][i], rcvdpkt->mincost[i]);
    ...
    //     printf("Summed to %d\n", summedPath);
    //     if( dt0.costs[rcvdpkt->sourceid][i] > rcvdpkt->mincost[i] && dt0.costs[rcvdpkt->sourceid][i] > rcvdpkt->mincost[i] && rcvdpkt->mincost[i] != 0 && summedPath > 0){
    //         updateOccur = 1;
